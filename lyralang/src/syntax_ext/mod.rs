...
impl SyntaxExtensionAnalyzer {
    ...
    fn handle_syntax_define(&mut self, call: &CallExpression, span: SourceSpan) {
        ...
        // Infer arity: count occurrences of `syntax_expand` in template string
        // (Stage 0 heuristic: each hole marker `$` in template counts as one).
    ...
    fn handle_syntax_expand(&mut self, call: &CallExpression, span: SourceSpan) {
        ...
        for i in 0..argument_count {
            let arg_snippet = expression_snippet(&call.arguments[i + 1]);
            ...
            let _ = arg_snippet; // used above for expanded form
